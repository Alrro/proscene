class Ceva {
  ...
  Ceva() {
    ...
    contraintePlane.setTranslationConstraint(AxisPlaneConstraint.Type.PLANE, new PVector(0.0f, 0.0f, 1.0f));
  ...
  PVector intersection(int i, int j, int k) {
    ...
    PVector f2=reperes[j].position();
    PVector f3=reperes[k].position();
    PVector f1f2=PVector.sub(f2, f1);
    PVector f0f3=PVector.sub(f3, reperes[0].position());     
    f1f2.normalize();
    f0f3.normalize();
  ...
  void  calculerIntersections() {
    ...
    sphere(2);
    popMatrix();
    PVector f0=reperes[0].position();
    PVector f1=reperes[1].position();
    PVector f2=reperes[2].position();
    PVector f3=reperes[3].position();
  ...
  void homothese() {
    ...
    PVector v2=h12.imagePoint(reperes[2].position());
  }
