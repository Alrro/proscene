class Arcad {
  ...
  Arcad (PVector ax) {
    ...
    //placements
    repere.setOrientation(new Quaternion(new PVector(1, 0, 0), -PI/4));
    repere.setPosition(axe);
    dragueur.setPosition(new PVector(1.2*axe.x, 1.2*axe.y, 1.2*axe.z));
    rotateur.setTranslation(new PVector(200*cos(QUARTER_PI), 200*sin(QUARTER_PI), 0));
  }
  ...
  void draw() {  
    ...
    ligne(repere.position(), rotateur.position());
    ...
    sym=PVector.mult(rotateur.translation(), -2);
    ...
    sphere(15);
    scene.drawAxis(100); 
    sym=rotateur.inverseCoordinatesOf(sym);
    popMatrix(); 
    ...
    PVector w= dragueur.translation().get(); 
    w.normalize();
    w.mult(lon);
    repere.setPosition(w);
    w.get().normalize();
    repere.setZAxis(w);
