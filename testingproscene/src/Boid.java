...
class Boid {
	...
	Boid(Scene s, PVector inPos) {
		...
		//frame.setInclination(frame.azimuth()+PApplet.QUARTER_PI);
		//frame.rotate(new Quaternion(new PVector(0,1,0), PApplet.HALF_PI));
	...
	void run(ArrayList bl) {
		...
		if (avoidWalls) {
			acc.add(PVector.mult(avoid(	new PVector(pos.x, ((flock3d)parent).flockHeight, pos.z), true), 5));
			acc.add(PVector.mult(avoid(new PVector(pos.x, 0, pos.z), true), 5));
			acc.add(PVector.mult(avoid(new PVector(((flock3d)parent).flockWidth, pos.y, pos.z),	true), 5));
			acc.add(PVector.mult(avoid(new PVector(0, pos.y, pos.z), true), 5));
			acc.add(PVector.mult(avoid(new PVector(pos.x, pos.y, 0), true), 5));
			acc.add(PVector.mult(avoid(new PVector(pos.x, pos.y, ((flock3d)parent).flockDepth), true), 5));
		}
		flock(bl);
		move();
		checkBounds();
	...
	void checkBounds() {
		if (pos.x > ((flock3d)parent).flockWidth)
			pos.x = 0;
		if (pos.x < 0)
			pos.x = ((flock3d)parent).flockWidth;
		if (pos.y > ((flock3d)parent).flockHeight)
			pos.y = 0;
		if (pos.y < 0)
			pos.y = ((flock3d)parent).flockHeight;
		if (pos.z > ((flock3d)parent).flockDepth)
			pos.z = 0;
		if (pos.z < 0)
			pos.z = ((flock3d)parent).flockDepth;
	}
