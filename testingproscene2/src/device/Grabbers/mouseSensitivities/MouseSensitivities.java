...
public class MouseSensitivities extends PApplet {
	private static final long serialVersionUID = 1L;

	public enum Sensitivity {
		ROTATION, TRANSLATION, WHEEL, SPINNING, TOSSING, SPINNING_FRICTION, TOSSING_FRICTION
	}
	...
	public void draw() {
		...
		// applyMatrix(scene.interactiveFrame().matrix()) is possible but
		// inefficient
	...
	void displayControls() {
		...
		if (isIFrame) {
			...
			displayText(
					"Interactive frame sensitivities (Press 'u' to view/set those of Camera frame)",
					xM, 30);
		} else {
			iFrame = scene.camera().frame();
			displayText(
					"Camera frame sensitivities (Press 'u' to view/set those of Interactive frame)",
					xM, 30);
		}
	...
	void changeSensitivity(InteractiveFrame iFrame, Sensitivity sens,
			boolean increase) {
		float step = 1;
		float res;
		switch (sens) {
