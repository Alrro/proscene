...
public class BasicUse extends PApplet {
	...
	public void draw() {		
		...
		//scene.drawArrow(new Vec(0,0,0), new Vec(50,50,50), 1);
	...
	public void keyPressed() {		
		if (key == 'c') {
			...
			scene.camera().setOrientation(new Quat(scene.camera().frame().zAxis(), radians));
		}
		
		if (key == 'w') {
			radians += PApplet.QUARTER_PI/4;
			scene.camera().setOrientation(new Quat(new Vec(0,0,1), radians));
		}
			
		if(key == 'y' || key == 'Y') {			
			Mat m_apply = new Mat( scene.getMatrix() );
			Mat m_applytrans = new Mat( scene.getMatrix() );			
			Mat p = new Mat( scene.getProjection() );
			/**
